...
func (*reportRepositoryImpl) Insert(report ReportsRow) (int64, error) {
	result, err := exec(
		...
		report.StartedAt,
		report.CompletedAt,
		report.UsersToCreateCount,
		report.UsersToUpdateCount,
		report.UsersToDeleteCount,
		report.GroupsToCreateCount,
		report.GroupsToUpdateCount,
		report.GroupsToDeleteCount,
	)
	if err != nil {
		return 0, err
	}
